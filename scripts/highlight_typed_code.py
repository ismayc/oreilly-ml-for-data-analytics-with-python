...
RULES = [
    ...
    # Exercise 1.1's cleaning block is four cells: copy, impute, engineer, dates.
    ("marketing_campaign = marketing_campaign_raw.copy()", "all", EX),
    ("marketing_campaign['Income'] = marketing_campaign['Income']", "all", EX),
    ('marketing_campaign["Kidhome"]', "all_except_totalspent", EX),
    ('marketing_campaign["Dt_Customer"] = pd.to_datetime', "all", EX),
    ...
    # Walkthrough 2.1 is eight one-purpose cells, matching the student scaffold.
    ("X = telco_churn[['AvgServiceUsageScore']]", "all", WT),
    ("X_train_raw, X_val_raw", "all", WT),
    ("X_train = scaler.fit_transform(X_train_raw)", "all", WT),
    ("lr = LinearRegression()", "all", WT),
    ('print(f"Intercept: {lr.intercept_:.2f}")', "all", WT),
    ("y_pred = lr.predict(X_val)", "all", WT),
    ("r2 = r2_score(y_val, y_pred)", "all", WT),
    ('print(f"R-squared: {r2:.2f}")', "all", WT),
    # Exercise 2.1 mirrors that shape.
    ("X_m = marketing_campaign[['Income']]", "all", EX),
    ("X_m_train_raw, X_m_val_raw", "all", EX),
    ("X_m_train = scaler_m.fit_transform(X_m_train_raw)", "all", EX),
    ("lr_m = LinearRegression()", "all", EX),
    ('print(f"Intercept: {lr_m.intercept_:.2f}")', "all", EX),
    ("y_m_pred = lr_m.predict(X_m_val)", "all", EX),
    ("r2_m = r2_score(y_m_val, y_m_pred)", "all", EX),
    ('print(f"R-squared: {r2_m:.2f}")', "all", EX),
    ("clf = RandomForestClassifier", "all", WT),
    # Exercise 2.2 is six cells: select, split, train, predict, score, matrix.
    ("features_m = ['Income', 'TotalSpent', 'TotalChildren']", "all", EX),
    ("X_m_train_class, X_m_val_class", "all", EX),
    ("clf_m = RandomForestClassifier", "all", EX),
    ("y_m_pred_class = clf_m.predict", "all", EX),
    ('print(f"Accuracy: {accuracy_score(y_m_val_class, y_m_pred_class):.2f}")', "all", EX),
    ("cm_marketing_response = pd.DataFrame", "all", EX),
    ("optimal_k = 3", "all", WT),
