...
# Reset index so the row labels are a clean 0, 1, 2, ...
...
```{python}
# Split FIRST, so the scaler never sees the validation rows
...
```{python}
# Fit the scaler on TRAINING data only, then apply that same
# transform to the validation set. fit_transform on train,
# transform (never fit) on validation.
...
# Same housekeeping as the walkthrough: a clean 0, 1, 2, ... index
X_m = X_m.reset_index(drop=True)
y_m = y_m.reset_index(drop=True)
```

```{python}
# Train-test split FIRST, so the scaler never sees the validation rows
X_m_train_raw, X_m_val_raw, y_m_train, y_m_val = train_test_split(
    X_m, y_m, test_size=0.2, random_state=2025
)
```
...
X_m_train = scaler_m.fit_transform(X_m_train_raw)
X_m_val = scaler_m.transform(X_m_val_raw)
```
...
# No scaling here: it moves inside the model, below.
...
```{python}
# Initialize logistic regression INSIDE a pipeline, so the scaler is
# refit on each cross-validation fold's training rows rather than once
# on everything. Same .fit()/.predict() API as a bare model.
...
# Train-test split. No scaling here: it goes inside the pipeline below,
# so each cross-validation fold scales using only its own training rows.
...
```{python}
# Same pipeline pattern as the walkthrough: scaler + model as one
# object, so cross_validate refits the scaler on every fold
logreg_m = make_pipeline(
    StandardScaler(),
    LogisticRegression(max_iter=1000, solver='liblinear')
)
```
