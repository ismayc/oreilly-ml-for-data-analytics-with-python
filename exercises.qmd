...
```{python}
# Make an 80/20 split FIRST, so the scaler never sees the validation rows

```

```{python}
# Fit the scaler on TRAINING data only, then apply that same transform
...
```{python}
# Same pipeline pattern as the walkthrough: scaler + model as one object
